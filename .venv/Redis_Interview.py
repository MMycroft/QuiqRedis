...
class Redis:
    ...
    """
    Explanation:
        Mapped strings to commands so that:
            Adding/removing commands is easy
            Debugging commands is easy
            DOC STRING is accessible. Documentation is exactly the same for developer and user alike
            DOC STRING can be used for unit testing?
    Improves: 
        add username (see below)
        figure out how to make commands a class attribute
    """
    def __init__(self):
    ...
    """
    Explanation:
        iterates through keys to check for removal, O(n) time
        insertion is O(1) time
    Improves: 
        use a linked list, order on insertion, O(n) time, removal is O(1) time
        OR
        use a heap to order keys, so that insertion and removal are O(log n) time
    """
    def clean(self):
        """
    ...
    """
    Explanation:
        executes the command, gets the return string from the command, prints return string
        return string includes line breaks, so print(, end='')
    Strengths:
        implementing new commands is easy
        debugging commands is easy, since errors are isolated
    Limitations:
        all commands must return a string
        all commands must handle an arbitrary number of arguments
        return string includes line breaks, which may allow for inconsistent/incorrect behavior
    Improves:
        clean up error handling
        make errors more generic: catch a general error e and push error strings down
        unexpected error may give confusing output
    """
    ...
    """
    Improves: 
        Printing format is weird do to DOC STRING. Improve display
    """
    ...
    """
    Improves: 
        perhaps use actual type checking for (nil) and (integer)
        since backend is python, just use python types: print(type(dict()).__name__)
    """
    ...
    """
    Improves: 
        does not delete from lifetimes
        change delete of lifetimes to use ordered linked list or heap
    """
    ...
    """
    Explanation: is [start, end] inclusive
    """
...
"""
Explanation:
    uses object attribute for program persistence
Improves: 
    use command line argument for UserName: python Redis.py MySuperCoolUserName
    in main, initialize redis = Redis(MySuperCoolUserName)
    if a file MySuperCoolUserName.txt exists: load and parse that file
    write redis contents to file MySuperCoolUserName.txt with header MySuperCoolUserName on termination

    use | instead of : to make string parsing easier
    implement a set, then use /start, ..., end/ instead of {start, ..., end} for string parsing
    implement an ordered set, then use <start, ..., end>
    EX.
        0 UserName
        1 'key1' | 'aStringEntry'
        2 'key2' | ['aListValue1', 'aListValue2', 'aListValue3']
        3 'key3' | {'aDictKey': 'aDictValue'}
        4 'key4' | /'aSetValue1', 'aSetValue2', 'aSetValue3'/
        5 'key5' | <'anOrderedSetValue1', 'anOrderedSetValue2', 'anOrderedSetValue3>'
"""
def main():
    redis = Redis()
    ...
    while redis.active:
        ...
        # ensure expired keys are deleted and execute command
        redis.clean()
